...
namespace ImGui
{
    ...
    template<typename T>
    IMGUI_API bool          Selectable(const char* label, T* enumValue, T ref, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0))
    {
        bool selected = *enumValue == ref;
        bool clicked = Selectable(label, &selected, flags, size);
        if(selected)
            *enumValue = ref;
        return clicked;
    }
